...
function WelfareRecordView:refreshView()
    ...
    for i=1,10 do
        ...
        if data then
            ...
            local year, month, day = Utils:getUTCDateYMD(data.time, true , GV_UTC_TIME_ZONE)
            local hour, min, sec = Utils:getUTCDateHMS(data.time, true , GV_UTC_TIME_ZONE)
            --local date = TFDate(data.time):tolocal()
            Label_time:setText(TextDataMgr:getText(800049, year, month, day, hour, min)..GV_UTC_TIME_STRING)
