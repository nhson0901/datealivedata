...
function AmusementPackMainView:onShow()
    ...
    if Utils:getLocalSettingValue("newYearenterWorldRoomFirst1") == "" and (not DatingDataMgr:triggerDating(self.__cname, "onShow")) and (not DatingDataMgr:getIsDating())  then
        FunctionDataMgr:jPersonInfoBase(4)
        Utils:setLocalSettingValue("newYearenterWorldRoomFirst1","true")
        return
    end
    local currentScene = Public:currentScene();
