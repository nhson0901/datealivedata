...
function AmusementPackControl:getFocusInterActionList( ... )
    ...
    for k,v in ipairs(self.focusNpcs) do
        ...
        local npcCfg = TabDataMgr:getData("WorldObjectMgr", npcData.decorateId)
        for k,v in ipairs(npcCfg.actionButtonId) do
            table.insert(interAction,{interActionId = v, actorPid = npcData.pid})
        end
    end
    return interAction
end
...
function AmusementPackControl:triggerNpcFunc( actionData )
    ...
    if actionData.pid then -- 统一播对象交互逻辑
        local actor = self:getActorNodeByPid(actionData.pid)
        local actorData = self:getActorDataByPid(actionData.pid)
    ...
    elseif interActionType == 5 then -- 建筑上播放表情
        self:operatePlayAction(actionData.interTypeParma.actionId)
    end
end
